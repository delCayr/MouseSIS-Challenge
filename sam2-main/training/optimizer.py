...
def freeze_param_modifier(
    scheduler_cfgs: List[List[Dict]],
    model,
    patterns: List[str] = (),
    freeze_blocks: Optional[Dict[str, int]] = None,  # e.g. {'image_encoder.trunk.blocks': 30}
    **kwargs
) -> List[List[Dict]]:
    # 方式1：按 pattern 匹配
    for name, param in model.named_parameters():
        for pattern in patterns:
            if fnmatch.fnmatchcase(name, pattern):
                ...
                print(f"[pattern] Freezing param: {name}")
                break

    # 方式2：冻结模块内前n层（如 trunk.blocks[:30]）
    if freeze_blocks is not None:
        for module_path, end_idx in freeze_blocks.items():
            # 递归找到模块
            submodule = rgetattr(model, module_path)  # 你已有rgetattr工具
            for i, block in enumerate(submodule):
                if i <= end_idx:
                    for p in block.parameters():
                        p.requires_grad = False
                    print(f"[block] Froze {module_path}[{i}]")
    ...
    for name, param in model.named_parameters():
        ...
        if not param.requires_grad:
            frozen += 1
            print(f"[frozen] {name}")
    print(f"Total params: {total}, Frozen: {frozen}, Trainable: {total - frozen}")
