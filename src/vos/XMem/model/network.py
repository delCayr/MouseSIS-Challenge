...
class XMem(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        ...
        self.key_proj = KeyProjection(1024, self.key_dim)  # 64

        self.decoder = Decoder(self.value_dim, self.hidden_dim)  # 512 64
    ...
    def segment(
        ...
        hidden_state, logits = self.decoder(
            *multi_scale_features, hidden_state, memory_readout, h_out=h_out #hidden_state：torch.Size([1, 1, 64, 45, 80])
        )  #logits: torch.Size([1, 1, 720, 1280])
    ...
    def init_hyperparameters(self, config, model_path=None, map_location=None):
        ...
        if model_path is not None:
            ...
            logging.debug(
                ...
                f"C^k={self.key_dim}, C^v={self.value_dim}, C^h={self.hidden_dim}" #64 512 64
            )
        else:
            model_weights = None
        ...
        config["key_dim"] = self.key_dim  # 64
        config["value_dim"] = self.value_dim  # 512
        config["hidden_dim"] = self.hidden_dim  # 64
